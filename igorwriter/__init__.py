...
class IgorWave5(object):
    ...
    def set_dimensionlablels(self, labels_list):
        """set_dimensionlablels sets the dimension labels of wave 

        Parameters
        ----------
        labels_list : list
            list of labels, must of equal length along given dimension for wave
        """
        self._dimension_labels = labels_list
        # if len(bunits) <= MAX_UNIT_CHARS:
        #     self._wave_header.dimUnits[dimint][:] = bunits + b'\x00' * (MAX_UNIT_CHARS + 1 - len(bunits))
        #     self._bin_header.dimEUnitsSize[dimint] = 0
        #     self._extended_dimension_units[dimint] = b''
    ...
    def save_itx(self, file, image=False):
        ...
        try:
            ...
            if self._dimension_labels != []:
                for n, label in enumerate(self._dimension_labels):
                    if label == '':
                        continue
                    else:
                        fp.write(f"X SetDimLabel 0, {n}, {label}, {name}\n")
        finally:
            if fp is not file:
                fp.close()
